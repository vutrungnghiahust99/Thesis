...
parser.add_argument('--use_big_head', type=int, choices=[0, 1], required=True)
...
# unchanged configs
parser.add_argument("--n_heads", type=int, choices=[1], default=1)
...
# make exp_folder
exp_folder = f'experiments/baseline/baseline_1head/{args.exp_name}'
...
if args.loss_name == 'gan1':
    ...
    discriminator = Discriminator(
        use_sigmoid=True,
        m=args.n_heads,
        use_big_head=args.use_big_head,
        use_mask_d=False)
else:
    loss = lsgan_loss
    discriminator = Discriminator(
...
for epoch in range(start_epoch, args.n_epochs):
    ...
    # statistic
    lossg_mean, lossg_std, lossd_mean, lossd_std, dx_mean, dx_std, dgz_mean, dgz_std = Metrics.compute_lossg_lossd_dx_dgz(
        loss, generator, discriminator, real_imgs_loader, args.bound, args.dist)
    entry_1 = [lossg_mean, lossg_std, lossd_mean, lossd_std, dx_mean, dx_std, dgz_mean, dgz_std]
    header_1 = ['lossg_mean', 'lossg_std', 'lossd_mean', 'lossd_std', 'dx_mean', 'dx_std', 'dgz_mean', 'dgz_std']
