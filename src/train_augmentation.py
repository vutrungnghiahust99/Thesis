...
for epoch in range(start_epoch, args.n_epochs):
    ...
    for (imgs, _, heads) in tqdm(dataloader):
        ...
        if args.augmentation:
            ...
            gen_imgs_ = torch.cat(
                [gen_imgs] + \
                [Augmentation.translation(gen_imgs) for _ in range(args.aug_times)] + \
                [Augmentation.add_guassian_noise(gen_imgs) for _ in range(args.aug_times)] + \
                [Augmentation.rand_cutout(gen_imgs) for _ in range(args.aug_times)] + \
                [Augmentation.rand_brightness(gen_imgs) for _ in range(args.aug_times)] + \
                [Augmentation.rand_contrast(gen_imgs) for _ in range(args.aug_times)] + \
                [Augmentation.rand_saturation(gen_imgs) for _ in range(args.aug_times)]
            )
            lossg = loss.compute_lossg_rf(discriminator, gen_imgs_, -1)
        ...
        for head_id in random_seq:
            ...
            if args.diff_data_for_heads:
                if not args.augmentation:
                    ...
                    mapping = {
                        0: Augmentation.add_guassian_noise,
                        1: Augmentation.translation,
                        2: Augmentation.rand_brightness,
                        3: Augmentation.rand_contrast,
                        4: Augmentation.rand_cutout,
                        5: Augmentation.rand_saturation,
                        6: Augmentation.add_guassian_noise,
                        7: Augmentation.translation,
                        8: Augmentation.rand_brightness,
                        9: Augmentation.rand_contrast
                    }
                    g_, r_ = get_gen_real_imgs_with_headID(gen_imgs.detach(), real_imgs, heads, head_id)
                    g = torch.cat([g_] + [mapping[head_id](g_) for _ in range(args.aug_times)])
                    r = torch.cat([r_] + [mapping[head_id](r_) for _ in range(args.aug_times)])
                    lossd = loss.compute_lossd_rf(discriminator, g, r, head_id)
            else:
                if not args.augmentation:
                    ...
                    g = torch.cat([gen_imgs.detach()] + [mapping[head_id](gen_imgs.detach()) for _ in range(args.aug_times)])
                    r = torch.cat([real_imgs] + [mapping[head_id](real_imgs) for _ in range(args.aug_times)])
                    lossd = loss.compute_lossd_rf(discriminator, g, r, head_id)
            lossd.backward()
            optimizer_D.step()
