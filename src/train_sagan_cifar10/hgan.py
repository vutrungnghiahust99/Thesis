...
from src.models.sagan_architecture_cifar10 import Generator, MDiscriminators
...
# make exp_folder
exp_folder = f'experiments/baseline/baseline_hgan/{args.exp_name}'
...
generator = Generator()
discriminator = MDiscriminators(m=args.n_heads)
...
dataloader = torch.utils.data.DataLoader(
    CIFAR10(
        root='data/cifar10/new_data/training.pt',
...
real_imgs_loader = torch.utils.data.DataLoader(
    CIFAR10_TEST(
        root='data/cifar10/new_data/test.pt',
        transform=transforms.Compose([
            transforms.Resize(args.img_size),
            transforms.ToTensor(),
            transforms.Normalize([0.5, 0.5, 0.5], [0.5, 0.5, 0.5])]
...
for epoch in range(start_epoch, args.n_epochs):
    ...
    lossg_mean, lossg_std, lossd_mean, lossd_std = Metrics.compute_lossg_lossd(
        HGANLoss, generator, discriminator, real_imgs_loader, args.bound, args.dist)
    ...
    fid = Metrics.compute_fid_rgb(generator, args.dist, args.bound)
    entry_4 = [fid]
    header_4 = ['fid_score']
