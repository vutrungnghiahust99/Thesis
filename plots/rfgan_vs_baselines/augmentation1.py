...
root1 = 'experiments/rfgan/augmentation'
exp1 = 'rfgan_lossgan1_bagging0_augmentation1_10heads_usemaskD'
exp2 = 'rfgan_lossgan1_bagging1_augmentation1_10heads_usemaskD'
exp3 = 'experiments/baseline/baseline_1head/baseline1head_lossgan1_augmentation1_usebighead0/baseline1head_lossgan1_augmentation1_usebighead0.csv'
exp4 = 'experiments/baseline/baseline_1head/baseline1head_lossgan1_augmentation1_usebighead1/baseline1head_lossgan1_augmentation1_usebighead1.csv'
exp5 = 'experiments/baseline/baseline_mheads/baselinemheads_lossgan1_augmentation1/baselinemheads_lossgan1_augmentation1.csv'
exp6 = 'experiments/baseline/baseline_hgan/baselinehgan_losshgan_augmentation1/baselinehgan_losshgan_augmentation1.csv'
...
for i in range(6):
    axs[0, 2].plot(epochs, df[i]['dx_mean'], color=colors[i])
axs[0, 2].set_xlabel('epoch', fontsize='x-large')
axs[0, 2].set_ylabel(r'$D(x)$', fontsize='xx-large')

for i in range(6):
    axs[1, 0].plot(epochs, df[i]['dgz_mean'], color=colors[i], label=labels[i])
...
plt.tight_layout()
plt.savefig('plots/rfgan_vs_baselines/augmentation1.pdf')
